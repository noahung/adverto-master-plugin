<?php
/**
 * LLM.txt Generator Class
 *
 * This class handles the generation of LLM.txt files for websites,
...
class Adverto_LLM_Generator {
    ...
    public function init_public_hooks($loader) {
        // Add rewrite rule for /llm.txt endpoint
    ...
    /**
     * Scan WordPress content for LLM.txt generation with live web crawling
     */
    public function scan_wordpress_content($options = array()) {
        ...
        foreach ($batches as $batch_index => $batch) {
            error_log("LLM Generator: Processing batch " . ($batch_index + 1) . " of " . count($batches));
            
            foreach ($batch as $post) {
                $url = get_permalink($post->ID);
                
                // Check if we need to skip due to time constraints
                if ($this->should_skip_due_to_time($processed, $total_posts, $options['processing_type'])) {
                    error_log("LLM Generator: Skipping remaining posts due to time constraints");
                    break 2; // Break out of both loops
                }
                
                // Crawl the live page content instead of using WordPress database
                error_log("LLM Generator: Crawling live page: " . $url);
                ...
                if ($options['processing_type'] !== 'basic') {
                    usleep(500000); // 0.5 second pause for AI processing
                }
            }
            
            // Clear any memory between batches
            if (function_exists('gc_collect_cycles')) {
                gc_collect_cycles();
            }
        }
        
        error_log("LLM Generator: Crawled " . count($content_data) . " pages successfully");
        return $content_data;
    }
    ...
    /**
     * Crawl live page content using WordPress HTTP API
     */
    private function crawl_page_content($url, $processing_type = 'basic') {
        ...
        // Use AI processing if requested and content is substantial
        if (in_array($processing_type, ['ai_summaries', 'key_points', 'structured']) && 
            !empty($extracted['content']) && strlen($extracted['content']) > 100) {
            
            $extracted['ai_summary'] = $this->generate_ai_content_summary($extracted['content'], $processing_type);
        }
    ...
    private function extract_meaningful_content($html) {
        ...
        // Try to extract main content areas (prioritized order)
        ...
        return $this->clean_extracted_content($extracted_content);
    }

    /**
     * Clean extracted HTML content
     */
    private function clean_extracted_content($content) {
        ...
        foreach ($lines as $line) {
            $line = trim($line);
            if (!empty($line) && strlen($line) > 5) { // Skip very short lines
                $cleaned_lines[] = $line;
            }
        }
        ...
        // Create excerpt (first 250 characters of meaningful content)
        $excerpt = $this->create_smart_excerpt($text_content, 250);
    ...
    /**
     * Create smart excerpt from content
     */
    private function create_smart_excerpt($content, $max_length = 250) {
        if (strlen($content) <= $max_length) {
            return $content;
        }
        
        // Try to break at sentence end
        $excerpt = substr($content, 0, $max_length);
        $last_sentence = strrpos($excerpt, '.');
        
        if ($last_sentence !== false && $last_sentence > ($max_length * 0.7)) {
            $excerpt = substr($excerpt, 0, $last_sentence + 1);
        } else {
    ...
    /**
     * Generate LLM.txt file with given options
     */
    public function generate_llm_file($options = array()) {
        try {
            ...
            $file_path = $upload_dir['basedir'] . '/llm.txt';
            ...
            if ($result === false) {
                ...
                return new WP_Error('file_write_error', __('Could not write LLM.txt file. Check file permissions.', 'adverto-master'));
            }
    ...
    private function generate_basic_llm_content($content_data, $options) {
        ...
        $content .= "*This LLM.txt file is generated by the award-winning team at [Adverto Media](https://advertomedia.co.uk/about-us/), a leading digital marketing agency based in Gloucester, England. We specialise in WordPress development, SEO optimisation, and intelligent content generation solutions.*\n\n";
        ...
        $content .= "This LLM.txt file was intelligently generated using advanced AI technology by **[Adverto Media](https://advertomedia.co.uk/about-us/)**, an award-winning digital marketing agency based in Gloucester, England.\n\n";
    ...
    private function generate_footer_section() {
        ...
        $content .= "---\n";
        $content .= "This LLM.txt file was automatically generated to provide structured information about this website's content for Large Language Models and AI applications.\n";
        
        return $content;
    }

    /**
     * Generate default LLM.txt if none exists
     */
    ...
    /**
     * Save LLM.txt content to database
     */
    ...
    /**
     * Get the file path for LLM.txt
     */
    private function get_file_path() {
        $upload_dir = wp_upload_dir();
        return $upload_dir['basedir'] . '/llm.txt';
    }

    /**
     * Get statistics about the current LLM.txt
     */
    ...
    public function init_admin_hooks($loader) {
        ...
        // Hook to serve public LLM.txt file
    ...
    /**
     * Handle AJAX request to generate LLM.txt file
     */
    public function ajax_generate_llm_txt() {
        ...
        try {
            ...
            if (is_wp_error($result)) {
                ...
                wp_send_json_success(array(
                    'message' => __('LLM.txt file generated successfully!', 'adverto-master'),
    ...
    /**
     * Handle AJAX request to download LLM.txt file
     */
    public function ajax_download_llm_txt() {
        $file_path = $this->get_file_path();
        
        if (!file_exists($file_path)) {
            wp_send_json_error(__('LLM.txt file not found', 'adverto-master'));
            return;
        }

        header('Content-Type: text/plain');
        header('Content-Disposition: attachment; filename="llm.txt"');
